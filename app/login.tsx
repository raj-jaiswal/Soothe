    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (msg.toLowerCase().includes('password') || msg.toLowerCase().includes('credentials')) {
          setErrors({ pw: msg });
        } else if (msg.toLowerCase().includes('user') || msg.toLowerCase().includes('found')) {
          setErrors({ id: msg });
        } else if (msg.toLowerCase().includes('verify') || msg.toLowerCase().includes('email')) {
      ...
      // Optionally store token here (AsyncStorage / SecureStore)
    ...
    <SafeAreaView style={styles.safeArea} edges={['top', 'bottom']}>
      ...
      <View style={styles.waveRow} pointerEvents="none">
        {WAVE_HEIGHTS.map((h, i) => (
          <View key={i} style={[styles.waveBar, { height: h, opacity: 0.06 + i * 0.01 }]} />
        ))}
      </View>
          ...
          <View style={styles.fieldWrapper}>
            ...
            <View style={[
              ...
              <TextInput
                ...
                value={identifier}
                onChangeText={(v) => { setIdentifier(v); setErrors((e) => ({ ...e, id: undefined, general: undefined })); }}
          ...
          <View style={[styles.fieldWrapper, { marginTop: 16 }]}>
            ...
            <View style={[
              ...
              <TextInput
                ...
                placeholderTextColor="#444"
                value={password}
                onChangeText={(v) => { setPassword(v); setErrors((e) => ({ ...e, pw: undefined, general: undefined })); }}
...
const styles = StyleSheet.create({
  ...
  glowLeft:        { position: 'absolute', top: 100, left: -80, width: 220, height: 220, borderRadius: 110, backgroundColor: 'rgba(139,92,246,0.14)' },
  glowRight:       { position: 'absolute', bottom: 120, right: -60, width: 180, height: 180, borderRadius: 90, backgroundColor: 'rgba(109,40,217,0.12)' },
  waveRow:         { position: 'absolute', bottom: 60, left: 0, right: 0, flexDirection: 'row', justifyContent: 'center', alignItems: 'flex-end' },
  waveBar:         { width: 14, backgroundColor: '#8B5CF6', borderRadius: 7, marginHorizontal: 5 },
  inner:           { flex: 1, paddingHorizontal: 24, paddingTop: 12, justifyContent: 'center' },
  header:          { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 48 },
  ...
  title:           { fontSize: 40, fontWeight: '800', color: '#FFFFFF', letterSpacing: -1.2, lineHeight: 48, marginBottom: 8 },
  titleAccent:     { color: '#a78bfa' },
  subtitle:        { fontSize: 14, color: '#666' },

  errorBanner:     { flexDirection: 'row', alignItems: 'center', backgroundColor: 'rgba(239,68,68,0.1)', borderWidth: 1, borderColor: 'rgba(239,68,68,0.25)', borderRadius: 10, paddingHorizontal: 12, paddingVertical: 10, marginBottom: 16 },
  ...
  inputRow:        { flexDirection: 'row', alignItems: 'center', backgroundColor: '#1a1a1a', borderRadius: 14, borderWidth: 1.5, borderColor: '#2a2a2a', paddingHorizontal: 14, height: 54 },
  ...
  forgotBtn:       { alignSelf: 'flex-end', marginTop: 10, marginBottom: 28, padding: 4 },
  forgotText:      { fontSize: 13, color: '#a78bfa', fontWeight: '600' },

  loginBtn:        { borderRadius: 16, overflow: 'hidden', shadowColor: '#7c3aed', shadowOffset: { width: 0, height: 8 }, shadowOpacity: 0.4, shadowRadius: 20, elevation: 12, marginBottom: 24 },
  loginGradient:   { paddingVertical: 17, alignItems: 'center', flexDirection: 'row', justifyContent: 'center', borderRadius: 16 },
  loginText:       { fontSize: 16, fontWeight: '700', color: '#FFFFFF', letterSpacing: 0.3 },
